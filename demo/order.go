 ...
 */
 ...
 func (orderDemo *OrderClient) VerifyToken(purchaseToken, productId string) {
	 bodyMap := map[string]string{"purchaseToken": purchaseToken, "productId": productId}
	 url := getOrderUrl() + "/applications/purchases/tokens/verify"
 ...
 func (orderDemo *OrderClient) CancelledListPurchase(endAt int64, startAt int64, maxRows int, productType int, continuationToken string) {
	 ...
	 url := getOrderUrl() + "/applications/v2/purchases/cancelledList"
	 bodyBytes, err := SendRequest(url, bodyMap)
	 if err != nil {
		 log.Printf("err is %s", err)
	 }
 ...
 func (orderDemo *OrderClient) ConfirmPurchase(purchaseToken, productId string) {
	 bodyMap := map[string]string{
		 "purchaseToken": purchaseToken,
		 "productId":     productId,
	 }
	 url := getOrderUrl() + "/applications/v2/purchases/confirm"
	 bodyBytes, err := SendRequest(url, bodyMap)
	 if err != nil {
		 log.Printf("err is %s", err)
	 }
